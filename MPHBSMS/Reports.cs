    ...
    public partial class Reports : Form
    {
        ...
        private void button7_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("No record found for Hospital Number: " + targetHospNo, "Marondera Provincial Hospital");
                    return;
                }
                ...
                using (FileStream fs = new FileStream(fullFilePath, FileMode.Create))
                {
                    ...
                    // --- LOGO ---
                    if (Properties.Resources.logo1 != null)
                    {
                        iTextSharp.text.Image logo = iTextSharp.text.Image.GetInstance(Properties.Resources.logo1, System.Drawing.Imaging.ImageFormat.Png);
                    ...
                    // SECTION 1: MASTER DATA (Admission & Discharge Timestamps)
                    ...
                    masterTable.SetWidths(new float[] { 15f, 35f, 15f, 35f });
                    ...
                    // Format Timestamps
                    string admStr = (mRow["admissionDate"] != DBNull.Value)
                                    ? Convert.ToDateTime(mRow["admissionDate"]).ToString("dd MMM yyyy HH:mm")
                                    : "N/A";

                    string disStr = "STILL ADMITTED";
                    if (mRow["isAdmitted"].ToString().Trim().ToUpper() == "NO")
                    {
                        disStr = (mRow["dischargeDate"] != DBNull.Value)
                                 ? Convert.ToDateTime(mRow["dischargeDate"]).ToString("dd MMM yyyy HH:mm")
                                 : "N/A";
                    }

                    masterTable.AddCell(new iTextSharp.text.Phrase("ADMISSION DATE:", tableHeaderFont));
                    masterTable.AddCell(new iTextSharp.text.Phrase(admStr, regFont));
                    masterTable.AddCell(new iTextSharp.text.Phrase("DISCHARGE DATE:", tableHeaderFont));
                    masterTable.AddCell(new iTextSharp.text.Phrase(disStr, regFont));
                    ...
                    foreach (DataRow row in historyData.Rows)
                    {
                        ...
                        string mDate = row["MovementDateTime"] != DBNull.Value
                                       ? Convert.ToDateTime(row["MovementDateTime"]).ToString("dd/MM/yyyy HH:mm")
                                       : "INITIAL ADM";

                        historyTable.AddCell(new iTextSharp.text.Phrase(mDate, smallFont));
                    ...
                    // SECTION 3: BED DAYS CALCULATION
                    if (mRow["admissionDate"] != DBNull.Value)
                    {
                        DateTime admDate = Convert.ToDateTime(mRow["admissionDate"]);
                        DateTime endDate = DateTime.Now;

                        if (mRow["isAdmitted"].ToString().Trim().ToUpper() == "NO" && mRow["dischargeDate"] != DBNull.Value)
                        {
                            endDate = Convert.ToDateTime(mRow["dischargeDate"]);
                        }
            ...
            {
                MessageBox.Show("Critical Error: " + ex.Message, "Marondera Provincial Hospital");
            }
        }
...
public void ExportSummaryToPDF(string startDateStr, string endDateStr, string currentUser)
{
    ...
    catch (Exception ex)
    {
        ...
        private void button23_Click(object sender, EventArgs e)
        {
            try
            {
                // 1. Validation: Ensure a file is selected in the list
                ...
                // 2. Construct the exact path (Matches C:\Users\...\Documents\...)
                string fileName = listBox1.SelectedItem.ToString();
                string documentsPath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
                string logsFolder = Path.Combine(documentsPath, "Marondera Provincial Hospital Bed Statistics Reports", "SystemLogs");
                string filePath = Path.Combine(logsFolder, fileName);

                // 3. Check if the file actually exists on the disk
                if (File.Exists(filePath))
                {
                    // 4. Open the raw .txt file using the system's default text editor (Notepad)
                    System.Diagnostics.Process.Start(new System.Diagnostics.ProcessStartInfo(filePath)
                    {
                        UseShellExecute = true
                    });

                    // Optional: Log that the file was viewed for audit purposes
                    MPHBSMS.LogActivity("SYSTEM: Viewed raw log file " + fileName);
                }
                else
                {
                    // If the file is missing, tell the user exactly where it should have been
                    MessageBox.Show("Could not find the file at:\n" + filePath, "File Error");

                    // Optional: Open the folder so the user can see what IS there
            ...
            {
                MessageBox.Show("Error opening log file: " + ex.Message, "Marondera Provincial Hospital");
            }
        }
        ...
        private void button21_Click(object sender, EventArgs e)
        {
            ...
            string inputID = textBox19.Text.Trim();
            string documentsPath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
            ...
            // Statistics categories matching your table's [category] field
            string[] categories = { "Admission", "Discharge", "Inter Ward Transfer In", "Inter Ward Transfer Out", "Death" };
            int[] counts = new int[5];
            int grandTotal = 0;

            try
            {
                // --- 2. VALIDATION (Dates removed for Lifetime Audit) ---
                if (string.IsNullOrEmpty(inputID))
                {
                    MessageBox.Show("Please enter a Staff ID Number to generate the report.", "Marondera Provincial Hospital");
                    return;
                }

                // --- 3. DATABASE MAPPING: ID NUMBER -> FULL NAME ---
                using (OleDbConnection con = new OleDbConnection(DatabaseHelper.ConnectionString))
                {
                    con.Open();

                    // Based on your userAccounts table: mapping idNumber to Name
                    string lookupSql = "SELECT [name] FROM [userAccounts] WHERE [idNumber] = ?";
                    using (OleDbCommand lookupCmd = new OleDbCommand(lookupSql, con))
                    {
                        lookupCmd.Parameters.Add("?", OleDbType.VarChar).Value = inputID;
                        object result = lookupCmd.ExecuteScalar();

                        if (result == null || result == DBNull.Value)
                        {
                            MessageBox.Show("No staff account found with ID Number: " + inputID, "Search Error");
                            return;
                        }
                        staffFullName = result.ToString();
                    }

                    // --- 4. DATABASE CALCULATION (Using Staff Name) ---
                    for (int i = 0; i < categories.Length; i++)
                    {
                        string sql = "SELECT COUNT(*) FROM [tblPatientMovement] WHERE [category] = ? AND [enteredBy] = ?";

                        using (OleDbCommand cmd = new OleDbCommand(sql, con))
                        {
                            cmd.Parameters.Add("?", OleDbType.VarChar).Value = categories[i];
                            cmd.Parameters.Add("?", OleDbType.VarChar).Value = staffFullName;

                            counts[i] = Convert.ToInt32(cmd.ExecuteScalar());
                            grandTotal += counts[i];
                        }
                    }
                }
                ...
                // --- 5. FOLDER SETUP ---
                mainFolder = Path.Combine(documentsPath, "Marondera Provincial Hospital Bed Statistics Reports");
                subFolder = Path.Combine(mainFolder, "User Lifetime Reports");
                if (!Directory.Exists(subFolder)) Directory.CreateDirectory(subFolder);

                string cleanName = string.Join("_", staffFullName.Split(Path.GetInvalidFileNameChars()));
                fullFilePath = Path.Combine(subFolder, "LifetimeAudit_" + cleanName + "_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".pdf");
                ...
                using (FileStream fs = new FileStream(fullFilePath, FileMode.Create))
                {
                    ...
                    cb.SetColorFill(new iTextSharp.text.BaseColor(245, 245, 245));
                    cb.ShowTextAligned(iTextSharp.text.Element.ALIGN_CENTER, "OVERALL USER AUDIT", doc.PageSize.Width / 2, doc.PageSize.Height / 2, 45);
                    cb.EndText();
                    ...
                    // Fonts
                    var titleFont = iTextSharp.text.FontFactory.GetFont("Helvetica-Bold", 16);
                    var boldFont = iTextSharp.text.FontFactory.GetFont("Helvetica-Bold", 10);
                    var regFont = iTextSharp.text.FontFactory.GetFont("Helvetica", 10);
                    ...
                    header.Alignment = iTextSharp.text.Element.ALIGN_CENTER;
                    doc.Add(header);
                    doc.Add(new iTextSharp.text.Paragraph("LIFETIME USER PERFORMANCE REPORT", boldFont) { Alignment = iTextSharp.text.Element.ALIGN_CENTER });
                    doc.Add(new iTextSharp.text.Paragraph("\n" + new string('_', 85) + "\n\n"));

                    // Metadata
                    doc.Add(new iTextSharp.text.Paragraph("STAFF NAME: " + staffFullName, boldFont));
                    doc.Add(new iTextSharp.text.Paragraph("ID NUMBER: " + inputID, regFont));
                    doc.Add(new iTextSharp.text.Paragraph("SCOPE: All Recorded Actions (Lifetime)", regFont));
                    doc.Add(new iTextSharp.text.Paragraph("GENERATED: " + DateTime.Now.ToString("F"), regFont));
                    doc.Add(new iTextSharp.text.Paragraph("\n"));
                    ...
                    table.AddCell(new iTextSharp.text.pdf.PdfPCell(new iTextSharp.text.Phrase("Activity Category", boldFont)) { BackgroundColor = iTextSharp.text.BaseColor.LIGHT_GRAY });
                    table.AddCell(new iTextSharp.text.pdf.PdfPCell(new iTextSharp.text.Phrase("Total Count", boldFont)) { BackgroundColor = iTextSharp.text.BaseColor.LIGHT_GRAY });
                    table.AddCell(new iTextSharp.text.pdf.PdfPCell(new iTextSharp.text.Phrase("% Contribution", boldFont)) { BackgroundColor = iTextSharp.text.BaseColor.LIGHT_GRAY });

                    for (int i = 0; i < categories.Length; i++)
                    {
                        double perc = (grandTotal > 0) ? ((double)counts[i] / grandTotal) * 100 : 0;
                        table.AddCell(new iTextSharp.text.Phrase(categories[i], regFont));
                        table.AddCell(new iTextSharp.text.Phrase(counts[i].ToString(), regFont));
                        table.AddCell(new iTextSharp.text.Phrase(perc.ToString("0.0") + "%", regFont));
                    }

                    iTextSharp.text.pdf.PdfPCell totalCell = new iTextSharp.text.pdf.PdfPCell(new iTextSharp.text.Phrase("TOTAL CUMULATIVE ACTIONS", boldFont));
                    totalCell.HorizontalAlignment = iTextSharp.text.Element.ALIGN_RIGHT;
                    table.AddCell(totalCell);
                    table.AddCell(new iTextSharp.text.Phrase(grandTotal.ToString(), boldFont));
                    table.AddCell(new iTextSharp.text.Phrase("100%", boldFont));
        ...
        private void button22_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (FileStream fs = new FileStream(fullPath, FileMode.Create))
                {
                    ...
                    if (Properties.Resources.logo1 != null)
                    {
                        // Convert Resource Image to iTextSharp Image
                        iTextSharp.text.Image logo = iTextSharp.text.Image.GetInstance(Properties.Resources.logo1, System.Drawing.Imaging.ImageFormat.Png);
                        logo.ScaleToFit(80f, 80f);
                        // Positioned at top left for landscape layout
                        logo.SetAbsolutePosition(40f, doc.PageSize.Height - 90f);
                        doc.Add(logo);
                    }
                    ...
                    // Header Text (Indented to allow room for logo)
                    iTextSharp.text.Paragraph p1 = new iTextSharp.text.Paragraph("MARONDERA PROVINCIAL HOSPITAL", titleFont);
                    p1.IndentationLeft = 100f;
                    doc.Add(p1);

                    iTextSharp.text.Paragraph p2 = new iTextSharp.text.Paragraph("USER ACTIVITY AUDIT REPORT", iTextSharp.text.FontFactory.GetFont("Helvetica-Bold", 12));
                    p2.IndentationLeft = 100f;
                    doc.Add(p2);

                    doc.Add(new iTextSharp.text.Paragraph("\n\n")); // Added spacing for logo area
                    ...
                    table.AddCell(new iTextSharp.text.pdf.PdfPCell(new iTextSharp.text.Phrase("Date/Time", tableHeaderFont)) { BackgroundColor = iTextSharp.text.BaseColor.LIGHT_GRAY });
                    table.AddCell(new iTextSharp.text.pdf.PdfPCell(new iTextSharp.text.Phrase("User Identity", tableHeaderFont)) { BackgroundColor = iTextSharp.text.BaseColor.LIGHT_GRAY });
                    table.AddCell(new iTextSharp.text.pdf.PdfPCell(new iTextSharp.text.Phrase("Action Performed", tableHeaderFont)) { BackgroundColor = iTextSharp.text.BaseColor.LIGHT_GRAY });
                    ...
                    {
                        // Search match across ID, Name, or Surname
                        bool isUserAction = (line.Contains(idMatch) ||
                                            (!string.IsNullOrEmpty(nameMatch) && line.Contains(nameMatch)) ||
                                            (!string.IsNullOrEmpty(surnameMatch) && line.Contains(surnameMatch)));
